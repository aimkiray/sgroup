...
public class UserDaoImpl extends BaseDao implements UserDao{
    ...
    public int addUser(User user) {
        int result = 0;
        String sql = "INSERT INTO user(user_name, user_password, real_name, email, gender, sign_date) VALUES (?,?,?,?,?,?)";
    ...
    public int UpdateUser(User user) {
        int result = 0;
        String sql = "UPDATE user set user_name = ?, user_password = ?, real_name = ?, email = ?, gender = ?, sign_date = ? WHERE user_id = ?";
    ...
    public List<User> getAllUser(ResultSet resultSet) {
        ...
        try {
            while (resultSet.next()) {
                ...
                user.setSignDate(resultSet.getDate("sign_date"));
                users.add(user);
            }
        } catch (SQLException e) {
            e.printStackTrace();
